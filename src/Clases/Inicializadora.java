...
public class Inicializadora {
	...
	public static void inicializar(ArrayList<Cliente> clientes,ArrayList<User> usuarios,ArrayList<Contrato> contratos,ArrayList<Agencia> agencias, ArrayList<Plazo_Deposito> plazos,ArrayList<CuentaBancaria> cuentas ){
    	
		loadUsers(usuarios);
    	loadAgencias(agencias);
    	loadPlazos(plazos);
    	loadContratos(contratos);
    	loadCuentas(cuentas,contratos,plazos);	
    	loadClientes(clientes,cuentas);
    }   
	
    //FUNCIONES DE PRUEBA DE DATOS
    
	public static void loadClientes(ArrayList<Cliente> clientes,ArrayList<CuentaBancaria>cuentas) {
        ...
        for (int i = 0; i < 20; i++) {
        	...
        	clientes.add(aux);
        	aux.getCuentas().add(cuentas.get(cont++));
        	aux.getCuentas().add(cuentas.get(cont ++));
        	aux.getCuentas().add(cuentas.get(cont ++));
        	aux.getCuentas().add(cuentas.get(cont ++));
        }
	...
	private static void loadCuentas(ArrayList<CuentaBancaria> cuentas,ArrayList<Contrato> contratos ,ArrayList<Plazo_Deposito> plazos) {
		int cont =0;
		...
		for(int i = 0 ; i < 20 ; i++){
			...
			cuentas.add(new C_Plazo_Fijo(cuentasPlazoFijo[i], beneficiarios[cont++], "CUP", plazos.get(i),cantidades[i]));
		}
